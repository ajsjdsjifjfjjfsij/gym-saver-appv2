...
export const ShareableGymCard = forwardRef<HTMLDivElement, ShareableGymCardProps>(({ gym, resolvedPhotoUrl, isFetchingPhotoUrl, onImageLoaded }, ref) => {
    ...
    return (
        ...
        >
            {/* Top Half: Massive Full-Bleed Gym Image */}
            <div className="w-full h-[600px] relative shrink-0" style={{ backgroundColor: '#18181B', borderBottom: '6px solid #6BD85E' }}>
                {base64Image ? (
                    <img
            ...
            <div className="flex-1 w-full p-12 flex flex-col justify-between relative z-10" style={{ backgroundColor: '#0A0A0A' }}>
                ...
                <div className="flex justify-between items-start gap-8 w-full">
                    ...
                    <div className="flex flex-col gap-4 flex-1 max-w-[65%]">
                        <h3 className="font-black text-[4rem] leading-[1.1] tracking-tighter uppercase break-words" style={{ color: '#6BD85E' }}>
                            {gym.name}
                        </h3>
                        {gym.address && (
                ...
                <div className="flex items-end justify-between w-full pb-4">
                    ...
                    <div className="flex flex-col gap-4 items-start">
                        <div className="flex items-center justify-center rounded-[2rem] px-10 py-6" style={{ backgroundColor: '#6BD85E', boxShadow: '0 10px 40px rgba(107, 216, 94, 0.4)' }}>
                            <div className="flex items-center gap-4">
                                <BookmarkCheck className="h-12 w-12" style={{ color: '#000000' }} />
                                <div className="flex items-baseline gap-2">
                                    <span className="text-[6.5rem] font-black leading-none tracking-tighter" style={{ color: '#000000' }}>
                    ...
                    <div className="flex flex-col items-end gap-4">
                        {(gym.is_24hr || gym.name.toLowerCase().includes("24") || gym.name.toLowerCase().includes("puregym") || gym.name.toLowerCase().includes("jd gyms") || gym.name.toLowerCase().includes("anytime")) && (
                            <span className="text-[1.8rem] font-black px-6 py-2 rounded-full uppercase tracking-widest border-2" style={{ borderColor: '#6BD85E', color: '#6BD85E' }}>
                                24/7
                            </span>
                        )}
                        {gym.latestOffer && typeof gym.latestOffer === "string" && !gym.latestOffer?.toLowerCase().includes("coming soon") && (
                            <span className="text-[2rem] font-black tracking-wide" style={{ color: '#FACC15' }}>
                                🔥 {gym.latestOffer}
                            </span>
                        )}
                    </div>
