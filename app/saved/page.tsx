...
export default function SavedGymsPage() {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            ...
            if (saved) {
                ...
                setSavedGyms(Array.isArray(parsed) ? parsed.slice(0, 3) : []);
            }
        }
    }, [])
    ...
    const gymsWithFacilities = useMemo(() => {
        return savedGyms.slice(0, 3).map(gym => {
    ...
    const removeGym = (id: string) => {
        const updated = savedGyms.filter(g => g.id !== id)
        setSavedGyms(updated)
        localStorage.setItem("savedGyms", JSON.stringify(updated))
    }

    // Show loading only during auth check
    if (authLoading) return null;

    // Guests can view saved gyms from localStorage
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-8 overflow-x-hidden">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-xs md:text-sm">Compare up to 3 gyms side-by-side.</p>
                    </div>
                </div>

                {gymsWithFacilities.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                        ...
                        <h2 className="text-xl font-bold">No gyms saved yet</h2>
                        <p className="text-slate-500 max-w-xs mx-auto">
                            Go back to the search page and bookmark some gyms to compare them here.
                        </p>
                        <Button asChild className="bg-[#6BD85E] text-black font-bold hover:bg-[#5bc250] rounded-full">
                            <Link href="/search">Find Gyms</Link>
                        </Button>
                    </div>
                ) : (
                                        ...
                                        <div className="mt-4 md:mt-auto pt-4 md:p-6">
                                            ...
                                            >
                                                <a href={`${gym.website}${gym.website?.includes('?') ? '&' : '?'}ref=gymsaver`} target="_blank" rel="noopener noreferrer">
                                                    Sign Up
                                                    <ExternalLink className="ml-1 md:ml-2 h-2 w-2 md:h-4 md:w-4 opacity-50 group-hover:opacity-100 transition-opacity" />
                                                </a>
                                            </Button>
                                        </div>
                                    </div>
